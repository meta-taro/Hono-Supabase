...
import { ReviewHelpfulVoteSupabaseRepository } from './review-helpful-vote.supabase-repository';
import { loadEnv, type RawEnv } from '@/shared/http/env';

// 実 Supabase ローカルに対して動く（`supabase start` 前提）。
// 分離戦略は review.supabase-repository.test.ts と同じ（@test-reviews.local / __test_reviews_）。
// 投票行は user_id / review_id への ON DELETE CASCADE で親（auth.users / reviews）削除に連動消去される。
...
const seedAuthUser = async (label: string): Promise<string> => {
  const email = `rv-${label}-${String(Date.now())}-${Math.random().toString(36).slice(2, 8)}${TEST_EMAIL_DOMAIN}`;
  const { data, error } = await sbAdmin.auth.admin.createUser({
    email,
    password: 'TestPassword123!',
    email_confirm: true,
    user_metadata: { name: `${TEST_CAKE_PREFIX}voter` },
  });
...
// 公開済みレビューを 1 件作って ReviewId を返す（投票対象）。
const seedPublishedReview = async (cakeId: string, authorId: string): Promise<Review> => {
  const reviewRepo = new ReviewSupabaseRepository(sbAdmin);
...
// reviews.helpful_count を直接読む（トリガ同期の検証用）。
...
describe('ReviewHelpfulVoteSupabaseRepository（実 Supabase ローカルに接続）', () => {
  beforeEach(cleanupTestRows);
  afterAll(cleanupTestRows);

  const repo = new ReviewHelpfulVoteSupabaseRepository(sbAdmin);

  it('add() でトリガが reviews.helpful_count を 1 に増やし、countByReview も 1 を返す', async () => {
    const author = await seedAuthUser('add-author');
    const voter = await seedAuthUser('add-voter');
    const cakeId = await seedCake('add');
    const review = await seedPublishedReview(cakeId, author);
  ...
  it('同一ユーザーの 2 回 add は ON CONFLICT DO NOTHING で二重カウントしない', async () => {
  ...
  it('複数ユーザーの投票はそれぞれ加算される', async () => {
    ...
    const cakeId = await seedCake('multi');
    const review = await seedPublishedReview(cakeId, author);
  ...
  it('remove() でトリガが helpful_count を減らす', async () => {
    ...
    const cakeId = await seedCake('rm');
    const review = await seedPublishedReview(cakeId, author);
    await repo.add(review.id, voter);
  ...
  it('未投票の remove は no-op（カウントは変化しない）', async () => {
    ...
    const cakeId = await seedCake('noop');
    const review = await seedPublishedReview(cakeId, author);
