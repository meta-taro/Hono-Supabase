...
- [ ] **Phase 9**: **観測・運用の質を上げる**（本番運用を想定したロギング・ヘルス・メトリクス・アラート）
  ...
  - [ ] **Step 2**: アクセスログミドルウェア — 入口で `start = Date.now()`、出口で `{method, path, status, duration_ms, userId}` を `info` で吐く。5xx は `error`、4xx は `warn`、それ以外 `info` で出し分け。Hono 標準の `logger()` は使わず自作（構造化ログに揃え、req スコープロガー経由で出すため）
